...
invertedRulesTable = {}; //FIXME: Remove this after debugging.

newComponent = null; //FIXME: Remove this after debugging.
...
require(['dojo/_base/declare'], function(declare) {
	...
	//// FIXME: I don't know what I was thinking when I added "appendTo".
	////    appendTo(parent) - appends the component to 'parent', a Zen
	////       component
	////    appendChild(child) - appends component 'child', a Zen
	////       component
	...
	constructor: function (id, domNode) {
	    console.debug("constructor: id => " + id + ", domNode => " + domNode);
            ...
            this.domNode = domNode || "[empty DomNodeCompon]";
            this.children = [];
	    zen.DomNodeCompon.domNodeCompons.push(this);
	},
	toString: function () { // Without this, we get '[object Object]'. FIXME: string should be '[DOM Node Compon ...
            var rep;
	...
	remove: function () {
	    ...
	    console.warn("this.remove(" + this + "), this.domNode => " + this.domNode +
			 ", zen.DomNodeCompon.domNodeCompons.length => " +
			 zen.DomNodeCompon.domNodeCompons.length);
	    ...
	    if (zen.registry.DomNodeCompon[this.id]) {
		delete zen.registry.DomNodeCompon[this.id];
	    }
	}
    });
});
...
(function (zen) {
	...
	*/
	if (attributes.style) {
	    ...
	    /* attributes.style = zen.fixCssDeclUrl(x, host); */
	    attributes.style = x;
	...
	*/
	...
	console.warn("zen.createSubtree: rule => " + rule);
        constructor = z.rule2ref(rule);
	//console.warn("zen.createSubtree: constructor => " + constructor);
    ...
    // FIXME: Uncomment this after removing the declaration above.
	...
	//console.warn("zen.rule2ref: ref => " + ref);
        if (!ref) {
	    //console.warn("zen.rule2ref: rule => " + rule);
            //ref = eval(rule);
	...
	if (node.nodeType == 3) {
	    ...
	    var j = 0, len = node.childNodes.length;
	    for (i = 0; i < len; i++) {
	...
	// FIXME: Uncomment this after removing the declaration above.
        //var newComponent;
	console.debug("renderTreeDeferred: parent => " + parent);
        z.walkZenSpec(
            tree,
            function(tree) {
	    ...
	    console.debug("zen.renderTreeDeferred: loaded, now calling createSubtree");
            newComponent = z.createSubtree(tree);
	    console.debug("zen.renderTreeDeferred: newComponent => " + newComponent + ", parent => " + parent);
            newComponent.appendMyselfToParent(parent);
	    console.debug("zen.renderTreeDeferred: after appending");
            z.startup();
	...
	/*
        ...
        dojo.forEach(
            ...
            function (child) { z.log("Destroying %s", child); child.remove(); }
        );
	*/
    };
	...
	console.debug("Entered zen.loadToolbox");
		...
		console.debug("Ajax result => " + result + ", zen.zenDiv => "
		    ...
		    console.debug("Get Zen toolbox: zenDiv => " + z.zenDiv);
    ...
    zen.getTestCSS = function () {
	dojo.io.iframe.send({
            url: "javascripts/zen/facebook.css",
            method: "GET",
            timeoutSeconds: 5,
            preventCache: true,
            handleAs: "text",
	...
	*/
	...
	console.warn("addOnLoad: filled invertedRulesTable");
	...
	console.warn("addOnLoad: filled shortcutsTable");
    });
    ...
    z.getRemotePageHandle = function (iframeID, iframeDoc) {
	require(["dojo/dom", "dojo/_base/window", "dojo/query"], function(dom, win, query) {
	    win.withDoc(iframeDoc || document, function() {
		ifr = dom.byId(iframeID);
		if (ifr) {
		    idoc = ifr.contentDocument;
		    console.log("ifr => " + ifr + ", idoc => " + idoc);
		    jsonGlobal = ifr.contentWindow; // Get the global scope object from the frame.
		} else {
		    console.error("Could not find " + iframeID + " in " + iframeDoc);
		}
	    });
	});
	return jsonGlobal;
    ...
    // Use zen.getRemotePageHandle before using this.
    zen.byId = function (id) {
	return idoc.getElementById(id);
    }

    // Use zen.getRemotePageHandle before using this.
    zen.byTag = function (tag) {
	return idoc.getElementsByTagName(tag)
    }

    // Use zen.getRemotePageHandle before using this.
    zen.byClass = function (klass) {
	return idoc.getElementsByClassName(klass)
    }

    // Use zen.getRemotePageHandle before using this.
    zen.styleSheet = function() {
	ss = idoc.styleSheets[0];
	rules = ss.cssRules;
	console.debug("ss => " + ss + ", rules => " + rules);
	return rules;
    }

    // Use zen.getRemotePageHandle before using this.
    zen.cssSelector = function(selectorText) {
	zen.styleSheet();
	return dojo.filter(rules, function(rule) { return selectorText == rule.selectorText; });
    } 

    // Use zen.getRemotePageHandle before using this.
    zen.cssSelector1 = function(selectorText) {
	zen.styleSheet();
	return dojo.filter(rules, function(rule) { return rule.selectorText.indexOf(selectorText) >= 0; });
    } 

    console.warn("Define zen.copyRemotePage");
    z.copyRemotePage = function (jsonIframeElement, targetIframeElement) {
	...
	require(["dojo/dom", "dojo/_base/window", "dojo/query"], function(dom, win, query) {
	    //jsonIframeDoc = dom.byId(jsonIframe).contentWindow.document;
	    //targetIframeDoc = dom.byId(targetIframe).contentWindow.document;
	    jsonIframeDoc = jsonIframeElement.contentWindow.document;
	    targetIframeDoc = targetIframeElement.contentWindow.document;
	    win.withDoc(jsonIframeDoc, function() {
		host = dojo.byId("remoteHost").textContent;
		z.remoteHost = host;
	    ...
	    win.withDoc(targetIframeDoc,  function() {
		...
		remoteCssFiles.forEach(
			...
			cssRules.forEach(
				...
				*/
				...
				//		  ", decls => " + decls);
				try {
				    if (selectorText.indexOf('lfloat') >= 0) {
					alert(selectorText + " { " + decls + " }");
				    }
				    dojox.html.insertCssRule(selectorText, decls);
				    cssRuleCount++;
				} catch(err) {
				    console.error("Failed CSS rule insertion: " + selectorText + " " + decls);
				    alert("Failed CSS rule insertion: " + selectorText + " " + decls);
				}
			    }
			);
		    }
...
zen.DomNodeCompon.destroyAll = function() {
    dojo.forEach(zen.DomNodeCompon.domNodeCompons,
		 function(domNodeCompon) {
		     if (domNodeCompon && domNodeCompon.getDomNode().tagName == "BODY") {
			 console.info("Calling " + domNodeCompon + ".destroy (with " +
				      domNodeCompon.children.length + " children)");
			 domNodeCompon.remove();
		     }
		 })
}

zen.DomNodeCompon.byId = function(id) {
    return zen.registry.DomNodeCompon[id];
}
